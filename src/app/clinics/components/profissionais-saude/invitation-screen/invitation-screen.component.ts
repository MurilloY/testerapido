...
import { Clinicurl } from 'src/app/clinics/returns/invite_url_verify.return';


@Component({
  selector: 'app-invitation-screen',
  templateUrl: './invitation-screen.component.html',
  styleUrls: ['./invitation-screen.component.scss']
})
export class InvitationScreenComponent implements OnInit {
  ...
  updateLinkProfessional(url:string) {

    this.clinicService.updateLinkProfessional(url).subscribe(data => {
      console.log(data)
          ...
          for (let i = 0; i < data.prof_clinic.days.length; i = i + 1) {
            ...
            disp.push(
              new FormGroup({
                ...
                presencial: new FormControl((data.prof_clinic.days[i].app_type == 1 || data.prof_clinic.days[i].app_type == 3) ? true : false, [Validators.required]),
                online: new FormControl((data.prof_clinic.days[i].app_type == 2 || data.prof_clinic.days[i].app_type == 3) ? true : false, [Validators.required]),
                days: new FormGroup({
